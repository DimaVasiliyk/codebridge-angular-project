...
export class HomePageState {
  ...
  @Selector()
  static getAllPages(state: HomepageStateModel): any {
    return state.pages;
  }

  @Selector()
  static getCount(state: HomepageStateModel): any {
    return state.countPages;
  }

  @Selector()
  static getFilterPage(state: HomepageStateModel): any {
  ...
  public checkCharacterCount(ctx: StateContext<HomepageStateModel>, { }: СheckCharacterCount) {
    ...
    if (letter) {
      ...
      for (let i = 0; i < finalPage.length; i++) {
        const titleMatches = finalPage[i].title.includes(letter);
        const summaryMatches = finalPage[i].summary.includes(letter);

        if (titleMatches || summaryMatches) {
          const spannedWord = `<span class="highlight" style='color:red'>${letter}</span>`;
          
          if (titleMatches) {
            const startIndex = finalPage[i].title.indexOf(letter);
            if (startIndex !== -1) {          
            ...
            const startIndex = finalPage[i].summary.indexOf(letter);
            if (startIndex !== -1) {          
